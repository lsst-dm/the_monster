    ...
    def run(self,
        ...
        for cat_info_target in self.target_catalog_info_class_list:
            ...
            for band in target_bands:
                if (band == "u") & (target_system_name != "SDSS"):
                    # for u band where we are not using null transform (null is SDSS to SDSS)
                    # we want to create monster and then use monster DES g and r band to transform to u band
                ...
                if band in bands:
                    if band == "u":
                        if target_system_name not in ['SDSS']:
                            raise ValueError("this portion of the code should only be the null transform for u band")
                        # since this is the null transform we dont need to apply a spline
                        # I am struggling to figure out how to get the gaiaxpu in since it either has only u-band or will
                        # propogate other bands ... open to input
                    ...
                    if band == "u":
                        # if u band we are only doing null transform in this part. So, no color selection.
                        flag = flux_not_nan
                    else: 
                        colors = cat_info.get_transformed_mag_colors(cat_stars, band)
                    ...
                    if len(cat_stars_selected) > 0:
                        ...
                        idx1, idx2 = esutil.numpy_util.match(gaia_stars_all['id'],cat_stars_selected['GaiaDR3_id'])
                        ...
                        gaia_stars_all[f"monster_{target_system_name}_{band}_source_flag"][idx1] = cat_info.flag
        
        if self.do_u_band_slr & ("u" in self.all_bands):
            # for u band slr we use des g and r bands to transform to sdss u band
            target_system_name = 'SDSS'
            band = "u"
            # we should have all of the sdss u band in monster format at this point 
            ...
            #only want to insert values that are not nan and are nan in gaia_stars_all
        ...
        if len(target_systems_u_transform) > 0:
            for target_system_name, band in target_systems_u_transform:
                ...
                orig_flux = gaia_stars_all[flux_col]
                orig_flux_err = gaia_stars_all[flux_col + 'Err']
                orig_flags = gaia_stars_all[f"monster_SDSS_u_source_flag"]
                ...
                # colors = cat_info.get_transformed_mag_colors(gaia_stars_all, band)
                # selected = (colors >= -10) & (colors <= 10)
